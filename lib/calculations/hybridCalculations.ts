...
} from "../utils/hybridConfig";
import { format, addMonths, parseISO } from "date-fns";

export function calculateHybridProjections(
  ...
  const selectedAddons = config.selectedAddons
    .map((id) => getFeatureAddon(id))
    .filter(Boolean) as FeatureAddon[];
  ...
  const tenantGrowthRate =
    ((finalTenants / initialTenants) ** (1 / 12) - 1) * 100;
  ...
  const avgMonthlyRevenuePerTenant =
    totalRevenue / 12 / ((initialTenants + finalTenants) / 2);
  ...
  const totalNewTenants = monthlyData.reduce(
    (sum, month) => sum + month.newTenants,
    0,
  );
...
export function exportHybridToCSV(
  config: HybridPricingData,
  ...
  const summaryRows = [
    ...
    [
      ...
      "",
      "",
      results.growthMetrics.tenantGrowthRate,
    ],
    [
      "User Growth Rate (%)",
      "",
      ...
      "",
      "",
      results.growthMetrics.userGrowthRate,
    ],
