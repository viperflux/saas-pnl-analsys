...
import { DEFAULT_GROWTH_SCENARIOS } from '@/lib/config';
...
export default function GrowthScenarioManager({
  ...
  const [scenarios, setScenarios] = useState<ExtendedGrowthScenario[]>(DEFAULT_GROWTH_SCENARIOS);
  ...
  const handleCustomScenarioChange = (field: keyof ExtendedGrowthScenario, value: any) => {
  ...
  const applyPattern = (pattern: 'steady' | 'seasonal' | 'growth' | 'decline') => {
    let newRates: number[] = [];
    ...
    switch (pattern) {
      ...
      case 'seasonal':
        ...
        newRates = Array(projectionMonths).fill(0).map((_, i) => seasonalPattern[i % 12]);
        break;
      case 'growth':
        newRates = Array(projectionMonths).fill(0).map((_, i) => Math.min(5 + i * 0.5, 20));
        break;
      case 'decline':
        newRates = Array(projectionMonths).fill(0).map((_, i) => Math.max(15 - i * 0.3, 2));
        break;
    }
    ...
    <div className={`space-y-4 ${className}`}>
      ...
      {showCustomEditor && (
        <div className="border border-gray-200 dark:border-gray-700 rounded-lg p-4 space-y-4">
          <h4 className="font-medium text-gray-900 dark:text-white">Custom Growth Scenario</h4>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => handleCustomScenarioChange('description', e.target.value)}
                className="form-input"
          ...
          <div>
            ...
            <div className="grid grid-cols-4 md:grid-cols-6 lg:grid-cols-12 gap-2 max-h-64 overflow-y-auto">
              {customScenario.growthRates.slice(0, projectionMonths).map((rate, index) => (
                <div key={index} className="space-y-1">
                  ...
                  <input
                    type="number"
                    value={rate}
                    onChange={(e) => handleGrowthRateChange(index, Number(e.target.value))}
      ...
      {selectedScenario !== 'custom' && (
        <div className="text-xs text-gray-500 dark:text-gray-400">
          <p className="font-medium">Growth Pattern Preview (First 12 months):</p>
          <div className="flex flex-wrap gap-1 mt-1">
            {selectedScenarioData.growthRates.slice(0, 12).map((rate, index) => (
              <span
                key={index}
