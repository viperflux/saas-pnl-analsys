...
import { HybridPricingData, PricingTier, FeatureAddon, GrowthScenario } from '@/types';
import { PRICING_TIERS, FEATURE_ADDONS, GROWTH_SCENARIOS } from '@/lib/hybridConfig';
...
export default function HybridPricingConfigurator({ config, onChange }: HybridPricingConfiguratorProps) {
  const updateConfig = (field: keyof HybridPricingData, value: any) => {
    onChange({ ...config, [field]: value });
  };

  const updateFixedCosts = (field: keyof HybridPricingData['monthlyFixedCosts'], value: number) => {
    onChange({
      ...config,
      monthlyFixedCosts: {
  ...
  const selectedTier = PRICING_TIERS.find(tier => tier.id === config.selectedTier);
    ...
    <div className="space-y-8">
            ...
            {PRICING_TIERS.map((tier) => (
              ...
              >
                <div className="text-center">
                  <h3 className="font-semibold text-lg text-gray-900 dark:text-gray-100">{tier.name}</h3>
          ...
          {selectedTier && (
            <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-4">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                <div>
                  <span className="text-blue-600 dark:text-blue-400 font-medium">Base Fee:</span>
                  <div className="font-semibold">${selectedTier.baseFee}/month</div>
                </div>
                <div>
                  <span className="text-blue-600 dark:text-blue-400 font-medium">Included Users:</span>
                  <div className="font-semibold">{selectedTier.includedUsers}</div>
                </div>
                <div>
                  <span className="text-blue-600 dark:text-blue-400 font-medium">AI Credits:</span>
                  <div className="font-semibold">{selectedTier.aiCredits.toLocaleString()}</div>
                </div>
                <div>
                  <span className="text-blue-600 dark:text-blue-400 font-medium">Overage Rates:</span>
                  <div className="font-semibold">${selectedTier.perUserRate}/user, ${selectedTier.aiOverageRate}/AI</div>
                </div>
              </div>
            </div>
          )}
      ...
      <div className="card">
        ...
        <div className="card-body">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => updateConfig('initialTenants', parseInt(e.target.value) || 1)}
                className="form-input"
                placeholder="10"
              />
            </div>
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => updateConfig('avgUsersPerTenant', parseInt(e.target.value) || 1)}
                className="form-input"
                placeholder="15"
              />
            </div>
          ...
          <div className="mt-6">
            <label className="form-label">Avg AI Usage per Tenant (credits/month)</label>
            <div className="flex items-center space-x-4">
              <input
                ...
                onChange={(e) => updateConfig('avgAiUsagePerTenant', parseInt(e.target.value))}
        ...
        <div className="card-body">
              ...
              >
                ...
                <div className="space-y-1 text-xs">
                  <div className="flex justify-between">
                    <span className="text-gray-500 dark:text-gray-500">Tenant Growth:</span>
                    <span className="font-medium">{(scenario.tenantGrowthRate * 100).toFixed(0)}%/mo</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-500 dark:text-gray-500">Churn Rate:</span>
                    <span className="font-medium">{(scenario.churnRate * 100).toFixed(0)}%/mo</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-500 dark:text-gray-500">Users/Tenant:</span>
                    <span className="font-medium">{scenario.usersPerTenant}</span>
                  </div>
                </div>
              </div>
            ))}
          ...
          {selectedScenario && (
            <div className="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg p-4">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                <div>
                  <span className="text-green-600 dark:text-green-400 font-medium">Tenant Growth:</span>
                  <div className="font-semibold">{(selectedScenario.tenantGrowthRate * 100).toFixed(1)}%/month</div>
                </div>
                <div>
                  <span className="text-green-600 dark:text-green-400 font-medium">User Growth:</span>
                  <div className="font-semibold">{(selectedScenario.userGrowthRate * 100).toFixed(1)}%/month</div>
                </div>
                <div>
                  <span className="text-green-600 dark:text-green-400 font-medium">AI Growth:</span>
                  <div className="font-semibold">{(selectedScenario.aiGrowthRate * 100).toFixed(1)}%/month</div>
                </div>
                <div>
                  <span className="text-green-600 dark:text-green-400 font-medium">Churn Rate:</span>
                  <div className="font-semibold">{(selectedScenario.churnRate * 100).toFixed(1)}%/month</div>
                </div>
              </div>
            </div>
          )}
        ...
        <div className="card-body">
          ...
          {config.selectedAddons.length > 0 && (
            <div className="mt-6 bg-purple-50 dark:bg-purple-900/20 border border-purple-200 dark:border-purple-800 rounded-lg p-4">
              <h4 className="font-medium text-purple-900 dark:text-purple-100 mb-2">Selected Add-ons</h4>
              <div className="space-y-2">
                {config.selectedAddons.map(addonId => {
                  const addon = FEATURE_ADDONS.find(a => a.id === addonId);
                  return addon ? (
                    <div key={addon.id} className="flex justify-between items-center text-sm">
                      <span className="text-purple-700 dark:text-purple-300">{addon.name}</span>
                ...
                <div className="border-t border-purple-200 dark:border-purple-800 pt-2 mt-2">
                  <div className="flex justify-between items-center font-semibold">
                    ...
                    <span>${config.selectedAddons.reduce((sum, addonId) => {
                      const addon = FEATURE_ADDONS.find(a => a.id === addonId);
